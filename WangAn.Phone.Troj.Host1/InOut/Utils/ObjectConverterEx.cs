...
namespace InOut.Utils
{
    ...
    public static class ObjectConverterEx
    {
        ...
        private static bool AddItemToList(IList oldList, IList newList, Type elementType, JavaScriptSerializerEx serializer, bool throwOnError)
        {
            ...
            foreach (Object propertyValue in oldList)
            {
                ...
                {
                    int temp = int.Parse(propertyValue.ToString());
                    if (temp < 0)
                    {
                        temp = 255 + temp;
                    }
                    convertedObject = (byte) temp;
                }
                else
                {
        ...
        private static bool AssignToPropertyOrField(object propertyValue, object o, string memberName, JavaScriptSerializerEx serializer, bool throwOnError)
        {
            ...
            PropertyInfo propInfo = GetPropertyInfo(serverType, memberName);
            //serverType.GetProperty(memberName, 
        ...
        private static Dictionary<Type, Dictionary<string, PropertyInfo>> Buffer = new Dictionary<Type, Dictionary<string, PropertyInfo>>();

        private static PropertyInfo GetPropertyInfo(Type serverType, string memberName)
        {
            if (!Buffer.ContainsKey(serverType))
            {
                var propertyDict = new Dictionary<string, PropertyInfo>();
                var properties = serverType.GetProperties(BindingFlags.Instance | BindingFlags.IgnoreCase | BindingFlags.Public);
                foreach (var propertyInfo in properties)
                {
                    var aliasAttribute = (AliasAttribute)propertyInfo.GetCustomAttribute(typeof(AliasAttribute));
                    propertyDict.Add(aliasAttribute != null ? aliasAttribute.Alias : propertyInfo.Name, propertyInfo);
                }
                Buffer.Add(serverType, propertyDict);
            }

            PropertyInfo info;
            Buffer[serverType].TryGetValue(memberName, out info);
            return info;
        }

        // Method that converts an IDictionary<string, object> to an object of the right type 
        private static bool ConvertDictionaryToObject(IDictionary<string, object> dictionary, Type type, JavaScriptSerializerEx serializer, bool throwOnError, out object convertedObject)
        {
            ...
            if (dictionary.TryGetValue(JavaScriptSerializerEx.ServerTypeFieldName, out s))
            {
                ...
                if (serverTypeName != null)
                {
                    ...
                    dictionary.Remove(JavaScriptSerializerEx.ServerTypeFieldName);
                }
            }
            ...
            if (IsGenericDictionary(type))
            {
                ...
                if (keyType != typeof(string) && keyType != typeof(object))
                {
                    if (throwOnError)
                    {
                        throw new InvalidOperationException(String.Format(CultureInfo.InvariantCulture, "不支持字典", type.FullName));
                    }
            ...
            if (type != null && !type.IsAssignableFrom(o.GetType()))
            {
                ...
                {
                    throw new MissingMethodException(String.Format(CultureInfo.InvariantCulture, "无构造函数", type.FullName));
                }

                throw new InvalidOperationException(String.Format(CultureInfo.InvariantCulture, "类型不匹配", type.FullName));
            }
        ...
        internal static object ConvertObjectToType(object o, Type type, JavaScriptSerializerEx serializer)
        {
            object convertedObject;
        ...
        private static bool ConvertObjectToTypeMain(object o, Type type, JavaScriptSerializerEx serializer, bool throwOnError, out object convertedObject)
        {
        ...
        private static bool ConvertObjectToTypeInternal(object o, Type type, JavaScriptSerializerEx serializer, bool throwOnError, out object convertedObject)
        {
                ...
                try
                {
                    ...
                    catch(Exception e)
                    {
                        if (type == typeof(byte))
                        {
                            ...
                            {
                                temp = 255 + temp;
                                convertedObject = converter.ConvertFromInvariantString(temp.ToString());
                                return true;
                            }
                        }
                        throw;
            ...
            if (throwOnError)
            {
                throw new InvalidOperationException(String.Format(CultureInfo.CurrentCulture, "类型不匹配", o.GetType(), type));
            }
            else
            {
                convertedObject = null;
        ...
        private static bool ConvertListToObject(IList list, Type type, JavaScriptSerializerEx serializer, bool throwOnError, out IList convertedList)
        {
                ...
                if (strongTypedEnumerable.IsAssignableFrom(type))
                {
                    ...
                    else
                    {
                        ...
                        if (t.IsAssignableFrom(type))
                        {
                            if (throwOnError)
                            {
                                throw new InvalidOperationException(String.Format(CultureInfo.InvariantCulture, "不能创建该类型列表 ", type.FullName));
                            }
                            else
                            {
                                convertedList = null;
            ...
            if (throwOnError)
            {
                throw new InvalidOperationException(String.Format(
                    CultureInfo.CurrentCulture, "不支持数组类型", type.FullName));
            }
            else
            {
                convertedList = null;
        ...
        internal static bool IsClientInstantiatableType(Type t, JavaScriptSerializerEx serializer)
        {
        ...
        internal static bool TryConvertObjectToType(object o, Type type, JavaScriptSerializerEx serializer, out object convertedObject)
        {
